...
(map (fun x => (vec (fst x) MRS_zero ++ snd x)) L) is thus the hypersequent (|- \vec{r_0}.0, T_0 | ... | |- \vec{r_n}.0, T_n) *)
Lemma hmrr_Z_inv_gen P : forall L,
    HMR P (map (fun x => (vec (fst x) MRS_zero ++ snd x)) L) ->
    HMR P (map (fun x => (snd x)) L).
Proof.
  intros L.
  remember (map (fun x : list Rpos * list (Rpos * term) => vec (fst x) MRS_zero ++ snd x) L) as G.
  assert (Allperm G (map (fun x : list Rpos * list (Rpos * term) => vec (fst x) MRS_zero ++ snd x) L)) by (rewrite <- HeqG; clear; induction G; simpl; try now constructor).
  clear HeqG.
  ...
  - destruct L; inversion Hperm; subst.
    ...
    assert (MRS_zero <> (MRS_covar n)) as Hnc by now auto.
    assert (MRS_zero <> (MRS_var n)) as Hnv by now auto.
    destruct (perm_decomp_vec_neq_2 _ _ _ _ _ _ _ _ Hnc Hnv X) as [ [[[Ta Tb] Da ] Db] [H1' [[[H2' H3'] H4'] H5']]].
    apply hmrr_ex_seq with (vec s (MRS_covar n) ++ vec r (MRS_var n) ++ Db).
    { Permutation_Type_solve. }
  ...
  - destruct L; inversion Hperm; subst.
    ...
    apply hmrr_ex_seq with (vec c1 MRS_zero ++ T').
    { Permutation_Type_solve. }
    apply hmrr_Z.
  ...
  - destruct L; inversion Hperm; subst.
    simpl.
    destruct p as [r1 T1]; simpl in *.
    assert (MRS_zero <> A +S B) as Hneq by now auto.
  ...
  - destruct L; inversion Hperm; subst.
    simpl.
    destruct p as [r1 T1]; simpl in *.
    assert (MRS_zero <> r0 *S A) as Hneq by now auto.
    destruct (perm_decomp_vec_neq _ _ _ _ _ _ Hneq X) as [ [[[Ta Tb] Da ] Db] [H1' [[[H2' H3'] H4'] H5']]].
  ...
  - destruct L; inversion Hperm; subst.
    simpl.
    destruct p as [r1 T1]; simpl in *.
    assert (MRS_zero <> MRS_coone) as Hnc by now auto.
    assert (MRS_zero <> MRS_one) as Hnv by now auto.
    destruct (perm_decomp_vec_neq_2 _ _ _ _ _ _ _ _ Hnc Hnv X) as [ [[[Ta Tb] Da ] Db] [H1' [[[H2' H3'] H4'] H5']]].
    apply hmrr_ex_seq with (vec s MRS_coone ++ vec r MRS_one ++ Db).
    { Permutation_Type_solve. }
  ...
  - destruct L; [ | destruct L]; inversion Hperm; try inversion X0; subst.
    ...
    assert (MRS_zero <> MRS_one) as Hnv by now auto.
    destruct (perm_decomp_vec_neq_2 _ _ _ _ _ _ _ _ Hnc Hnv X) as [ [[[Ta Tb] Da ] Db] [H1' [[[H2' H3'] H4'] H5']]].
    apply hmrr_ex_seq with (vec s MRS_coone ++ vec r MRS_one ++ Db) ; [ Permutation_Type_solve | ].
    ...
    change ((vec s MRS_coone ++ vec r MRS_one ++ Db'') :: nil)
      with
        (map (fun x : list Rpos * list (Rpos * term) => snd x) ((nil, vec s MRS_coone ++ vec r MRS_one ++ Db'') :: nil)).
    apply IHpi.
    ...
    apply Permutation_Type_app_inv_l with (vec r MRS_one).
    apply Permutation_Type_app_inv_l with (vec s MRS_coone).
...
Proof.
  ...
  - destruct L; inversion Hperm; subst.
    simpl.
    destruct p as [r1 T1]; simpl in *.
    assert (A +S B <> (MRS_covar n)) as Hnc by now auto.
    assert (A +S B <> (MRS_var n)) as Hnv by now auto.
    destruct (perm_decomp_vec_neq_2 _ _ _ _ _ _ _ _ Hnc Hnv X) as [ [[[Ta Tb] Da ] Db] [H1' [[[H2' H3'] H4'] H5']]].
    apply hmrr_ex_seq with (vec s (MRS_covar n) ++ vec r (MRS_var n) ++ vec r1 A ++ vec r1 B ++ Db).
    { Permutation_Type_solve. }
    apply hmrr_ID; try assumption.
  ...
  - destruct L; inversion Hperm; subst.
    simpl.
    destruct p as [r1 T1]; simpl in *.
    assert (A +S B <> MRS_zero) as Hneq by now auto.
    destruct (perm_decomp_vec_neq _ _ _ _ _ _ Hneq X) as [ [[[Ta Tb] Da ] Db] [H1' [[[H2' H3'] H4'] H5']]].
    apply hmrr_ex_seq with (vec r MRS_zero ++ vec r1 A ++ vec r1 B ++ Db).
    { etransitivity; [ apply Permutation_Type_app_comm | ].
      rewrite <- ? app_assoc; repeat (apply Permutation_Type_app; try reflexivity).
      Permutation_Type_solve. }
  ...
  - destruct L; inversion Hperm; subst.
    simpl.
    destruct p as [r1 T1]; simpl in *.
    assert (A +S B <> MRS_coone) as Hnc by now auto.
    assert (A +S B <> MRS_one) as Hnv by now auto.
    destruct (perm_decomp_vec_neq_2 _ _ _ _ _ _ _ _ Hnc Hnv X) as [ [[[Ta Tb] Da ] Db] [H1' [[[H2' H3'] H4'] H5']]].
    apply hmrr_ex_seq with (vec s MRS_coone ++ vec r MRS_one ++ vec r1 A ++ vec r1 B ++ Db).
    { Permutation_Type_solve. }
    apply hmrr_one; try assumption.
    change ((vec r1 A ++ vec r1 B ++ Db) :: map (fun x : list Rpos * list (Rpos * term) => vec (fst x) A ++ vec (fst x) B ++ snd x) L)
      with
  ...
  - destruct L; [ | destruct L]; inversion Hperm; try inversion X0; subst.
    ...
    2:{ symmetry in H4'; apply seq_diamond_perm_decomp in H4' as [D' H'].
        destruct D'; inversion H'. }
    apply hmrr_ex_seq with (vec s MRS_coone ++ vec r MRS_one ++ seq_diamond Db'') ; [ Permutation_Type_solve | ].
    apply hmrr_diamond; try assumption.
    change ((vec s MRS_coone ++ vec r MRS_one ++ Db'') :: nil)
      with
        (map (fun x : list Rpos * list (Rpos * term) => vec (fst x) A ++ vec (fst x) B ++ snd x) ((nil, vec s MRS_coone ++ vec r MRS_one ++ Db'') :: nil)).
    apply IHpi.
    apply Forall2_inf_cons ; [ | apply Forall2_inf_nil].
    simpl.
    apply Permutation_Type_app; try reflexivity.
...
Proof with try assumption.
  ...
  - destruct L; inversion Hperm; subst.
    simpl.
    destruct p as [r1 T1]; simpl in *.
    assert (a *S A <> (MRS_covar n)) as Hnc by now auto.
    assert (a *S A <> (MRS_var n)) as Hnv by now auto.
    destruct (perm_decomp_vec_neq_2 _ _ _ _ _ _ _ _ Hnc Hnv X) as [ [[[Ta Tb] Da ] Db] [H1' [[[H2' H3'] H4'] H5']]].
    apply hmrr_ex_seq with (vec s (MRS_covar n) ++ vec r (MRS_var n) ++ vec (mul_vec a r1) A  ++ Db).
    { Permutation_Type_solve. }
    apply hmrr_ID; try assumption.
  ...
  - destruct L; inversion Hperm; subst.
    simpl.
    destruct p as [r1 T1]; simpl in *.
    assert (a *S A <> MRS_zero) as Hneq by now auto.
    destruct (perm_decomp_vec_neq _ _ _ _ _ _ Hneq X) as [ [[[Ta Tb] Da ] Db] [H1' [[[H2' H3'] H4'] H5']]].
    apply hmrr_ex_seq with (vec r MRS_zero ++ vec (mul_vec a r1) A ++ Db).
    { etransitivity; [ apply Permutation_Type_app_comm | ].
      rewrite <- ? app_assoc; repeat (apply Permutation_Type_app; try reflexivity).
      Permutation_Type_solve. }
    apply hmrr_Z; try assumption.
  ...
  - destruct L; inversion Hperm; subst.
    simpl.
    destruct p as [r1 T1]; simpl in *.
    assert (a *S A <> MRS_coone) as Hnc by now auto.
    assert (a *S A <> MRS_one) as Hnv by now auto.
    destruct (perm_decomp_vec_neq_2 _ _ _ _ _ _ _ _ Hnc Hnv X) as [ [[[Ta Tb] Da ] Db] [H1' [[[H2' H3'] H4'] H5']]].
    apply hmrr_ex_seq with (vec s MRS_coone ++ vec r MRS_one ++ vec (mul_vec a r1) A ++ Db).
    { Permutation_Type_solve. }
    apply hmrr_one; try assumption.
    change ((vec (mul_vec a r1) A ++ Db) :: map (fun x : list Rpos * list (Rpos * term) => vec (mul_vec a (fst x)) A ++ snd x) L)
      with
  ...
  - destruct L; [ | destruct L]; inversion Hperm; try inversion X0; subst.
    ...
    change ((vec s MRS_coone ++ vec r MRS_one ++ Db'') :: nil)
      with
        (map (fun x : list Rpos * list (Rpos * term) => vec (mul_vec a (fst x)) A ++ snd x) ((nil, vec s MRS_coone ++ vec r MRS_one ++ Db'') :: nil)).
    apply IHpi.
    apply Forall2_inf_cons ; [ | apply Forall2_inf_nil].
    simpl.
    apply Permutation_Type_app; try reflexivity.
...
Proof.
  ...
  - destruct L; inversion Hperm; subst.
    ...
    assert (A \/S B <> (MRS_var n)) as Hnv by now auto.
    destruct (perm_decomp_vec_neq_2 _ _ _ _ _ _ _ _ Hnc Hnv X) as [ [[[Ta Tb] Da ] Db] [H1' [[[H2' H3'] H4'] H5']]].
    apply hmrr_ex_seq with (vec s (MRS_covar n) ++ vec r (MRS_var n) ++ vec r1 A ++ Db).
    { Permutation_Type_solve. }
    apply hmrr_ID; try assumption.
    eapply hmrr_ex_hseq ; [ rewrite app_comm_cons; apply Permutation_Type_app_comm | ].
    simpl.
    apply hmrr_ex_seq with (vec s (MRS_covar n) ++ vec r (MRS_var n) ++ vec r1 B ++ Db).
    { Permutation_Type_solve. }
    apply hmrr_ID; try assumption.
  ...
  - destruct L; inversion Hperm; subst.
    ...
    assert (A \/S B <> MRS_zero) as Hneq by now auto.
    destruct (perm_decomp_vec_neq _ _ _ _ _ _ Hneq X) as [ [[[Ta Tb] Da ] Db] [H1' [[[H2' H3'] H4'] H5']]].
    apply hmrr_ex_seq with (vec r MRS_zero ++ vec r1 A ++ Db).
    { Permutation_Type_solve. }
    apply hmrr_Z; try assumption.
    eapply hmrr_ex_hseq ; [ rewrite app_comm_cons; apply Permutation_Type_app_comm | ].
    simpl.
    apply hmrr_ex_seq with (vec r MRS_zero ++ vec r1 B ++ Db).
    { Permutation_Type_solve. }
    apply hmrr_Z; try assumption.
    eapply hmrr_ex_hseq; [ |  apply (IHpi ((r1 , Db) :: L))].
    { simpl.
  ...
  - destruct L; inversion Hperm; subst.
    ...
    assert (A \/S B <> MRS_one) as Hnv by now auto.
    destruct (perm_decomp_vec_neq_2 _ _ _ _ _ _ _ _ Hnc Hnv X) as [ [[[Ta Tb] Da ] Db] [H1' [[[H2' H3'] H4'] H5']]].
    apply hmrr_ex_seq with (vec s MRS_coone ++ vec r MRS_one ++ vec r1 A ++ Db).
    { Permutation_Type_solve. }
    apply hmrr_one; try assumption.
    ...
    apply hmrr_ex_seq with (vec s MRS_coone ++ vec r MRS_one ++ vec r1 B ++ Db) ; [ Permutation_Type_solve | ].
    apply hmrr_one; try assumption.
  ...
  - destruct L; [ | destruct L]; inversion Hperm; try inversion X0; subst.
    ...
    apply hmrr_diamond; try assumption.
    apply hmrr_C.
    change ((vec s MRS_coone ++ vec r MRS_one ++ Db'') :: (vec s MRS_coone ++ vec r MRS_one ++ Db'') :: nil) with
        (map (fun x => vec (fst x) A ++ snd x) ((nil, vec s MRS_coone ++ vec r MRS_one ++ Db'') :: nil) ++ map (fun x => vec (fst x) B ++ snd x) ((nil, vec s MRS_coone ++ vec r MRS_one ++ Db'') :: nil)).
    apply IHpi.
    apply Forall2_inf_cons ; [ | apply Forall2_inf_nil].
    simpl.
    apply Permutation_Type_app; try reflexivity.
...
Proof.
  ...
  - destruct L; [ | destruct L]; inversion Hperm; try inversion X0; subst.
    ...
    change ((vec s MRS_coone ++ vec r MRS_one ++ Db'') :: nil)
      with
        (map (fun x : list Rpos * list (Rpos * term) => vec (fst x) A ++ snd x) ((nil, vec s MRS_coone ++ vec r MRS_one ++ Db'') :: nil)).
    apply IHpi.
    apply Forall2_inf_cons ; [ | apply Forall2_inf_nil].
    simpl.
    apply Permutation_Type_app; try reflexivity.
...
Proof.
  ...
  - destruct L; inversion Hperm; subst.
    ...
    assert (A /\S B <> MRS_one) as Hnv by now auto.
    destruct (perm_decomp_vec_neq_2 _ _ _ _ _ _ _ _ Hnc Hnv X) as [ [[[Ta Tb] Da ] Db] [H1' [[[H2' H3'] H4'] H5']]].
    apply hmrr_ex_seq with (vec s MRS_coone ++ vec r MRS_one ++ vec r1 B ++ Db).
    { Permutation_Type_solve. }
    apply hmrr_one; try assumption.
    change ((vec r1 B ++ Db) :: map (fun x : list Rpos * list (Rpos * term) => vec (fst x) B ++ snd x) L)
      with
  ...
  - destruct L; [ | destruct L]; inversion Hperm; try inversion X0; subst.
    ...
    change ((vec s MRS_coone ++ vec r MRS_one ++ Db'') :: nil)
      with
        (map (fun x : list Rpos * list (Rpos * term) => vec (fst x) B ++ snd x) ((nil, vec s MRS_coone ++ vec r MRS_one ++ Db'') :: nil)).
    apply IHpi.
    apply Forall2_inf_cons ; [ | apply Forall2_inf_nil].
    simpl.
    apply Permutation_Type_app; try reflexivity.
...
Lemma hmrr_diamond_inv_gen : forall L,
    HMR_T_M (map (fun x => (vec (fst (fst x)) MRS_coone ++ vec (snd (fst x)) MRS_one ++ seq_diamond (snd x))) L) ->
    HMR_T_M (map (fun x => (vec (fst (fst x)) MRS_coone ++ vec (snd (fst x)) MRS_one ++ snd x)) L).
Proof.
  intros L.
  remember (map (fun x => (vec (fst (fst x)) MRS_coone ++ vec (snd (fst x)) MRS_one ++ seq_diamond (snd x))) L) as G.
  assert (Allperm G (map (fun x => (vec (fst (fst x)) MRS_coone ++ vec (snd (fst x)) MRS_one ++ seq_diamond (snd x))) L)) by (rewrite <- HeqG; clear; induction G; simpl; try now constructor).
  clear HeqG.
  intro pi; revert L X; induction pi; intros L Hperm.
  - destruct L ; [ | destruct L]; inversion Hperm; try inversion X0; subst.
    apply Permutation_Type_nil in X.
  ...
  - destruct L; inversion Hperm; subst.
    simpl; apply hmrr_C.
    change ((vec (fst (fst p)) MRS_coone ++ vec (snd (fst p)) MRS_one ++ snd p)
              :: (vec (fst (fst p)) MRS_coone ++ vec (snd (fst p)) MRS_one ++ snd p)
              :: map
              (fun x : list Rpos * list Rpos * list (Rpos * term) =>
                 vec (fst (fst x)) MRS_coone ++ vec (snd (fst x)) MRS_one ++ snd x) L)
      with (map (fun x => vec (fst (fst x)) MRS_coone ++ vec (snd (fst x)) MRS_one ++ snd x) (p :: p :: L)).
    apply IHpi.
  ...
  - destruct L ; [ | destruct L]; inversion Hperm; try inversion X0; subst.
    ...
    simpl in *.
    apply hmrr_S.
    apply hmrr_ex_seq with (vec (s1 ++ s2) MRS_coone ++ vec (r1 ++ r2) MRS_one ++ T1' ++ T2').
    { rewrite ? vec_app.
      Permutation_Type_solve. }
    change ((vec (s1 ++ s2) MRS_coone ++ vec (r1 ++ r2) MRS_one ++ T1' ++ T2')
              :: map
              (fun x : list Rpos * list Rpos * list (Rpos * term) =>
                 vec (fst (fst x)) MRS_coone ++ vec (snd (fst x)) MRS_one ++ snd x) L)
      with
        (map (fun x => vec (fst (fst x)) MRS_coone ++ vec (snd (fst x)) MRS_one ++ snd x) (((s1 ++ s2, r1 ++ r2) , T1' ++ T2') :: L)).
    apply IHpi.
  ...
  - destruct L; inversion Hperm; subst.
    ...
    apply hmrr_ex_seq with ((vec s1 MRS_coone ++ vec r1 MRS_one ++ D1') ++ (vec s2 MRS_coone ++ vec r2 MRS_one ++ D2')).
    { transitivity (vec (s1 ++ s2) MRS_coone ++ vec (r1 ++ r2) MRS_one ++ D1' ++ D2').
      - rewrite ? vec_app.
        Permutation_Type_solve.
      - repeat apply Permutation_Type_app; try apply vec_perm; try Permutation_Type_solve.
        ...
        transitivity (vec (s1 ++ s2) MRS_coone ++ vec (r1 ++ r2) MRS_one ++ (seq_diamond (D1' ++ D2'))).
        + repeat apply Permutation_Type_app; try apply vec_perm; try Permutation_Type_solve.
        + etransitivity ; [ | apply Permutation_Type_app ; symmetry; [ apply H4 | apply H5]].
          rewrite ? vec_app; rewrite seq_diamond_app.
          Permutation_Type_solve. }
    apply hmrr_M; try assumption.
    + change ((vec s1 MRS_coone ++ vec r1 MRS_one ++ D1')
                :: map
                (fun x : list Rpos * list Rpos * list (Rpos * term) =>
                   vec (fst (fst x)) MRS_coone ++ vec (snd (fst x)) MRS_one ++ snd x) L)
        with
          (map (fun x => vec (fst (fst x)) MRS_coone ++ vec (snd (fst x)) MRS_one ++ snd x) (((s1,r1),D1')::L)).
      apply IHpi1; simpl; apply Forall2_inf_cons; assumption.
    + change ((vec s2 MRS_coone ++ vec r2 MRS_one ++ D2')
                :: map
                (fun x : list Rpos * list Rpos * list (Rpos * term) =>
                   vec (fst (fst x)) MRS_coone ++ vec (snd (fst x)) MRS_one ++ snd x) L)
        with
          (map (fun x => vec (fst (fst x)) MRS_coone ++ vec (snd (fst x)) MRS_one ++ snd x) (((s2,r2),D2')::L)).
      apply IHpi2; simpl; apply Forall2_inf_cons; assumption.
  - destruct L; inversion Hperm; subst.
    destruct p as [r1 T1]; simpl in *.
    apply hmrr_T with r; try assumption.
    rewrite ? seq_mul_app; rewrite ? seq_mul_vec_mul_vec.
    change ((vec (mul_vec r (fst r1)) MRS_coone ++ vec (mul_vec r (snd r1)) MRS_one ++ seq_mul r T1)
              :: map
              (fun x : list Rpos * list Rpos * list (Rpos * term) =>
                 vec (fst (fst x)) MRS_coone ++ vec (snd (fst x)) MRS_one ++ snd x) L)
      with
        (map (fun x => vec (fst (fst x)) MRS_coone ++ vec (snd (fst x)) MRS_one ++ snd x) (((mul_vec r (fst r1), mul_vec r (snd r1)), seq_mul r T1) :: L)).
    apply IHpi.
    simpl; apply Forall2_inf_cons; try assumption.
  ...
  - destruct L; inversion Hperm; subst.
    ...
    destruct s.
    2:{ exfalso.
        assert (~ In (r0, MRS_covar n) (vec s1 MRS_coone ++ vec r1 MRS_one ++ seq_diamond T1)) as H.
        - intros Hin.
        ...
        - apply H.
          apply Permutation_Type_in with (vec (r0 :: s) (MRS_covar n) ++ vec r (MRS_var n) ++ T); try assumption.
          simpl; left.
          reflexivity. }
    destruct r.
    2:{ exfalso.
        assert (~ In (r, MRS_var n) (vec s1 MRS_coone ++ vec r1 MRS_one ++ seq_diamond T1)) as H.
        - intros Hin.
          apply in_app_or in Hin.
          case Hin ; [ | intros Hin2; apply in_app_or in Hin2; case Hin2].
          + clear.
          ...
          + apply seq_diamond_only_diamond.
            intros B; intros H; inversion H.
        - apply H.
          apply Permutation_Type_in with (vec (r :: r0) (MRS_var n) ++ T); try assumption.
          simpl; left.
          reflexivity. }
    simpl in *.
    change ((vec s1 MRS_coone ++ vec r1 MRS_one ++ T1)
              :: map
              (fun x : list Rpos * list Rpos * list (Rpos * term) =>
                 vec (fst (fst x)) MRS_coone ++ vec (snd (fst x)) MRS_one ++ snd x) L)
      with (map (fun x => vec (fst (fst x)) MRS_coone ++ vec (snd (fst x)) MRS_one ++ snd x) (((s1,r1),T1)::L)).
    apply IHpi.
    apply Forall2_inf_cons; try assumption.
  - destruct L; inversion Hperm; subst.
    destruct p as [[s1 r1] T1]; simpl in *.
    destruct r.
    2:{ exfalso.
        assert (~ In (r, MRS_zero) (vec s1 MRS_coone ++ vec r1 MRS_one ++ seq_diamond T1)) as H.
        - intros Hin.
          apply in_app_or in Hin.
          case Hin ; [ | intros Hin2; apply in_app_or in Hin2; case Hin2].
          + clear.
          ...
          + apply seq_diamond_only_diamond.
            intros B; intros H; inversion H.
        - apply H.
          apply Permutation_Type_in with (vec (r :: r0) MRS_zero ++ T); try assumption.
          simpl; left.
          reflexivity. }
    simpl in *.
    change ((vec s1 MRS_coone ++ vec r1 MRS_one ++ T1)
  ...
  - destruct L; inversion Hperm; subst.
    ...
    destruct r.
    2:{ exfalso.
        assert (~ In (r, A +S B) (vec s1 MRS_coone ++ vec r1 MRS_one ++ seq_diamond T1)) as H.
        - intros Hin.
          apply in_app_or in Hin.
          case Hin ; [ | intros Hin2; apply in_app_or in Hin2; case Hin2].
          + clear.
  ...
  - destruct L; inversion Hperm; subst.
    ...
    destruct r.
    2:{ exfalso.
        assert (~ In (r, r0 *S A) (vec s1 MRS_coone ++ vec r1 MRS_one ++ seq_diamond T1)) as H.
        - intros Hin.
          apply in_app_or in Hin.
          case Hin ; [ | intros Hin2; apply in_app_or in Hin2; case Hin2].
          + clear.
  ...
  - destruct L; inversion Hperm; subst.
    ...
    simpl in *.
    apply hmrr_C.
    change ((vec s1 MRS_coone ++ vec r1 MRS_one ++ T1) :: (vec s1 MRS_coone ++ vec r1 MRS_one ++ T1)
              :: map
              (fun x : list Rpos * list Rpos * list (Rpos * term) =>
                 vec (fst (fst x)) MRS_coone ++ vec (snd (fst x)) MRS_one ++ snd x) L)
      with (map (fun x => vec (fst (fst x)) MRS_coone ++ vec (snd (fst x)) MRS_one ++ snd x) (((s1,r1),T1)::((s1,r1),T1)::L)).
    apply IHpi.
  ...
  - destruct L; inversion Hperm; subst.
    destruct p as [[s1 r1] T1]; simpl in *.
    destruct (perm_decomp_vec_eq_2 T (seq_diamond T1) r1 s1 r s MRS_one MRS_coone) as [[[[[[[[a1 b1] c1] a2] b2] c2] T'] D'] [H1' [[[[[H2' H3'] H4'] H5'] H6']]]]; [ now auto | apply X | ].
    destruct c2.
    2:{ exfalso.
        refine (seq_diamond_only_diamond T1 MRS_coone r2 _ _).
        - intros B Heq; inversion Heq.
        - apply in_inf_in.
          apply Permutation_Type_in_inf with (vec (r2 :: c2) MRS_coone ++ vec c1 MRS_one ++ D'); try Permutation_Type_solve.
          left; reflexivity. }
    destruct c1.
    2:{ exfalso.
        refine (seq_diamond_only_diamond T1 MRS_one r2 _ _).
        - intros B Heq; inversion Heq.
        - apply in_inf_in.
          apply Permutation_Type_in_inf with (vec (r2 :: c1) MRS_one ++ D'); try Permutation_Type_solve.
          left; reflexivity. }
    simpl in *; rewrite app_nil_r in *.
    apply hmrr_ex_seq with (vec s MRS_coone ++ vec r MRS_one ++ vec a2 MRS_coone ++ vec a1 MRS_one ++ T1).
    { transitivity (vec (a2 ++ s) MRS_coone ++ vec (a1 ++ r) MRS_one ++ T1); [ | repeat (try apply Permutation_Type_app; try apply vec_perm; try Permutation_Type_solve) ].
      rewrite ? vec_app.
      Permutation_Type_solve. }
    apply hmrr_one; try assumption.
    change ((vec a2 MRS_coone ++ vec a1 MRS_one ++ T1)
              :: map
              (fun x : list Rpos * list Rpos * list (Rpos * term) =>
                 vec (fst (fst x)) MRS_coone ++ vec (snd (fst x)) MRS_one ++ snd x) L)
      with
        (map (fun x => vec (fst (fst x)) MRS_coone ++ vec (snd (fst x)) MRS_one ++ snd x) (((a2,a1), T1)::L)).
    apply IHpi.
    simpl; apply Forall2_inf_cons; try assumption.
    Permutation_Type_solve.    
  - destruct L ; [ | destruct L]; inversion Hperm; try inversion X0; subst.
    ...
    { clear - X.
      ...
      - simpl in *.
        ...
        exfalso.
        assert (~ In (r0, MRS_coone) (vec r MRS_one ++ seq_diamond T)) as H.
        + intros Hin; apply in_app_or in Hin; case Hin.
          ...
          * apply seq_diamond_only_diamond.
            intros B Heq; inversion Heq.
        + apply H.
          apply Permutation_Type_in with (vec (r0 :: s') MRS_coone ++ vec r' MRS_one ++ seq_diamond T'); try Permutation_Type_solve.
          left; reflexivity.
      - destruct (in_inf_split a s') as [[s1' s2'] H].
        + assert (In_inf (a , MRS_coone) (vec s' MRS_coone)).
          * destruct (in_inf_app_or (vec s' MRS_coone) (vec r' MRS_one ++ seq_diamond T') (a , MRS_coone)); try assumption.
            -- apply Permutation_Type_in_inf with (vec (a :: s) MRS_coone ++ vec r MRS_one ++ seq_diamond T); try Permutation_Type_solve; left; reflexivity.
            -- clear - i.
               exfalso.
               induction r'.
        ...
        + subst.
          ...
          apply Permutation_Type_skip.
          apply IHs.
          apply Permutation_Type_cons_inv with (a , MRS_coone); rewrite ? vec_app in Hperm; simpl in *.
    ...
    { clear - X Hperms.
      ...
      - simpl in *.
        ...
        exfalso.
        assert (~ In (r, MRS_one) (vec s MRS_coone ++ seq_diamond T)) as H.
        + intros Hin; apply in_app_or in Hin; case Hin.
          * intros H; clear - H.
          ...
          * apply seq_diamond_only_diamond.
            intros B Heq; inversion Heq.
        + apply H.
          apply Permutation_Type_in with (vec s' MRS_coone ++ vec (r :: r') MRS_one ++ seq_diamond T'); try Permutation_Type_solve.
          apply in_or_app; right.
          left; reflexivity.
      - destruct (in_inf_split a r') as [[r1' r2'] H].
        + assert (In_inf (a , MRS_one) (vec r' MRS_one)).
          * destruct (in_inf_app_or (vec s' MRS_coone) (vec r' MRS_one ++ seq_diamond T') (a , MRS_one)).
            -- apply Permutation_Type_in_inf with (vec s MRS_coone ++ vec (a :: r) MRS_one ++ seq_diamond T); try Permutation_Type_solve.
        ...
        + subst.
          ...
          apply Permutation_Type_skip.
          apply IHr.
          apply Permutation_Type_cons_inv with (a , MRS_one); rewrite ? vec_app in Hperm; simpl in *.
          rewrite vec_app.
  ...
  - destruct L; inversion Hperm; subst.
    ...
    change ((vec s MRS_coone ++ vec r MRS_one ++ T)
              :: map
              (fun x : list Rpos * list Rpos * list (Rpos * term) =>
                 vec (fst (fst x)) MRS_coone ++ vec (snd (fst x)) MRS_one ++ snd x) L)
      with
        (map (fun x => vec (fst (fst x)) MRS_coone ++ vec (snd (fst x)) MRS_one ++ snd x) (((s,r), T) :: L)).
    apply IHpi.
    simpl; apply Forall2_inf_cons; try assumption.
    Permutation_Type_solve.
  - destruct (Permutation_Type_Forall2_inf (Permutation_Type_sym p) Hperm).
...
Lemma hmrr_Z_inv : forall G T r,  HMR_T_M ((vec r MRS_zero ++ T) :: G) -> HMR_T_M (T :: G).
Proof.
  intros G.
  assert ({ L & prod
                  ( G = map (fun x : list Rpos * list (Rpos * term) => vec (fst x) MRS_zero ++ snd x) L)
...
Lemma hmrr_diamond_inv : forall T r s, HMR_T_M ((vec s MRS_coone ++ vec r MRS_one ++ seq_diamond T) :: nil) -> HMR_T_M ((vec s MRS_coone ++ vec r MRS_one ++ T) :: nil).
Proof.
  intros T r s pi.
  change ((vec s MRS_coone ++ vec r MRS_one ++ T) :: nil)
    with
      (map (fun x => vec (fst (fst x)) MRS_coone ++ vec (snd (fst x)) MRS_one ++ snd x) (((s,r),T) :: nil)).
