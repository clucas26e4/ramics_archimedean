...
Fixpoint p_seq_fst_non_basic_term (T : p_sequent) : (Poly * term) :=
  match T with
  | nil => (Poly_cst 0, MRS_one)
  | (a, A) :: T => if (0 <? MRS_outer_complexity_term A)%nat
                   then (a , A)
...
Proof.
  ...
  destruct a as [a A]; simpl in *.
  case_eq (0 <? MRS_outer_complexity_term A)%nat; intros H1; rewrite H1 in Hbt;
    (apply Nat.ltb_lt in H1 + apply Nat.ltb_nlt in H1).
  - apply is_basic_outer_complexity_0 in Hbt.
    simpl in Hbt.
    lia.
  - apply IHT; auto.
    intros H2.
    apply Hnbs.
    apply Forall_inf_cons; auto.
    apply is_basic_outer_complexity_0_inv.
    lia.
Qed.

Lemma p_seq_fst_non_basic_term_well_defined :
  forall val T,
    (0 < HMR_outer_complexity_p_seq T)%nat ->
...
Proof.
  ...
  destruct a as [a A].
  simpl.
  case_eq (0 <? MRS_outer_complexity_term A)%nat; intros H; auto; inversion Hwd; subst; auto.
  apply IHT; auto.
...
Proof.
  ...
  case_eq (0 <? MRS_outer_complexity_term A)%nat; intros H1;
...
Proof.
  ...
  simpl.
  case_eq (0 <? MRS_outer_complexity_term A)%nat; intros H; try apply Forall_inf_cons; try apply IHT; auto.
Qed.

Fixpoint p_hseq_p_seq_max_complexity (G : p_hypersequent) : p_sequent :=
  match G with
  | nil => nil
  | T :: G => if (fst (HMR_outer_complexity_p_hseq G) <=? HMR_outer_complexity_p_seq T)
              then T
...
Proof.
  ...
  simpl; case (fst (HMR_outer_complexity_p_hseq G) <=? HMR_outer_complexity_p_seq a); auto.
Qed.

Lemma p_hseq_p_seq_max_complexity_correct :
  forall G,
    HMR_outer_complexity_p_seq (p_hseq_p_seq_max_complexity G) = fst (HMR_outer_complexity_p_hseq G).
Proof.
  induction G; auto.
  simpl.
  case_eq (fst (HMR_outer_complexity_p_hseq G) <=? HMR_outer_complexity_p_seq a); intros H1;
    case_eq (HMR_outer_complexity_p_seq a =? fst (HMR_outer_complexity_p_hseq G)); intros H2;
      case_eq (HMR_outer_complexity_p_seq a <? fst (HMR_outer_complexity_p_hseq G))%nat; intros H3;
        simpl;
...
Proof.
  ...
  simpl; case (fst (HMR_outer_complexity_p_hseq G) <=? HMR_outer_complexity_p_seq a); try apply Forall_inf_cons; auto.
Qed.
...
Lemma p_hseq_put_non_basic_fst_modal_complexity :
  forall G,
    ...
    complexity_p_hseq (p_hseq_put_non_basic_fst G) = complexity_p_hseq G.
Proof.
  intros G Hnb.
  unfold p_hseq_put_non_basic_fst.
  rewrite complexity_perm_fst_seq with _ _ (p_hseq_p_seq_max_complexity G).
  2:{ symmetry; apply p_seq_put_non_basic_fst.
      ...
      apply H. }
  rewrite complexity_perm with _ G; auto.
...
Lemma p_hseq_put_non_basic_fst_HMR_complexity :
  forall G,
    (p_hseq_is_basic G -> False) ->
    HMR_outer_complexity_p_hseq (p_hseq_put_non_basic_fst G) = HMR_outer_complexity_p_hseq G.
Proof.
  intros G Hnb.
  apply same_complexity_HMR_outer_complexity.
...
Lemma p_hseq_put_non_basic_fst_well_defined :
  forall val G,
    (0 < fst (HMR_outer_complexity_p_hseq G))%nat ->
    p_hseq_well_defined val G ->
...
Proof.
  ...
  - inversion Hwd; inversion X; subst.
    ...
    2:{ apply pi. }
    revert pi;set (r := (existT (fun x : R => (0 <? x) = true) (eval_Poly val a) e)); intros pi.
    change ((r, MRS_zero) :: eval_p_sequent val l) with
        (hseq.vec (r :: nil) MRS_zero ++ eval_p_sequent val l).
    apply hmrr_Z.
    apply pi.
  - inversion Hwd; inversion X; subst.
    simpl in pi |- *.
...
Lemma apply_logical_rule_on_p_hypersequent_correct_inl :
  forall G G1 n,
    snd (fst (complexity_p_hseq G)) = S n ->
    apply_logical_rule_on_p_hypersequent (p_hseq_put_non_basic_fst G) = inl G1 ->
    complexity_p_hseq G1 <3 complexity_p_hseq G.
Proof.
  intros G G1 n H1 H2.
  unfold complexity_p_hseq in H1.
  simpl in H1.
  ...
  { exfalso.
    ...
    intros Hnb.
    apply p_hseq_is_basic_outer_complexity_0 in Hnb; lia. }
  destruct l.
  ...
  { apply p_hseq_put_non_basic_fst_correct with G a l H; auto.
    intros Hb.
    apply p_hseq_is_basic_outer_complexity_0 in Hb.
    lia. }
  ...
  - rewrite <- (p_hseq_put_non_basic_fst_modal_complexity G).
    2:{ intros Hnb.
        apply p_hseq_is_basic_outer_complexity_0 in Hnb; lia. }
    rewrite <- HeqH.
    change ((a, MRS_zero) :: l) with (vec (a :: nil) MRS_zero ++ l).
    apply hmrr_Z_decrease_modal_complexity ; [ intros H'; inversion H' | ].
    simpl vec; simpl app.
    rewrite HeqH.
    unfold p_hseq_put_non_basic_fst in *.
    rewrite HMR_outer_complexity_perm_fst_seq with _ _ (p_hseq_p_seq_max_complexity G).
    2:{ symmetry; apply p_seq_put_non_basic_fst.
        intros Hb.
        apply p_seq_is_basic_complexity_0 in Hb.
    ...
    rewrite <-p_hseq_p_seq_max_complexity_correct.
    rewrite outer_complexity_p_seq_perm with (p_hseq_p_seq_max_complexity G) (p_seq_fst_non_basic_term (p_hseq_p_seq_max_complexity G) :: p_seq_without_fst_non_basic_term (p_hseq_p_seq_max_complexity G)).
...
Lemma apply_logical_rule_on_p_hypersequent_correct_inr_r :
  forall G G1 G2 n,
    snd (fst (complexity_p_hseq G)) = S n ->
    apply_logical_rule_on_p_hypersequent (p_hseq_put_non_basic_fst G) = inr (G1 , G2) ->
    complexity_p_hseq G2 <3 complexity_p_hseq G.
Proof.
  intros G G1 G2 n H1 H2.
  unfold complexity_p_hseq in H1.
  simpl in H1.
