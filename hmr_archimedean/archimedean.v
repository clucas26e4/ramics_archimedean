...
Require Import RL.hmr_archimedean.lambda_prop_one.
...
(** * Modal free case *)
(** ** Theorem 7 with the additional information that t_i = 1 . *)
(** Useful when dealing with sequences, to ensure that t_{i,n} = 1 for all elements of the sequence *)
Definition p_def_lambda_prop_modal_free_one_fixed i G val := 
  { L &
...
Lemma p_lambda_prop_modal_free_one_seq_fixed: forall G (un : nat -> val_UniformSpace),
    (forall n, p_def_lambda_prop_modal_free_one G (un n)) ->
    {' (phi , i) : _ & prod (subseq_support phi)
                            (forall n, p_def_lambda_prop_modal_free_one_fixed i G (un (phi n)))}.
Proof.
  intros G un H.
...
Lemma p_lambda_prop_modal_free_one_lim : forall G (un : nat -> val_UniformSpace) (ul : val_UniformSpace),
    is_lim_seq un ul ->
    (forall n, p_def_lambda_prop_modal_free_one G (un n)) ->
    p_def_lambda_prop_modal_free_one G ul.
Proof.
  enough (forall G i (un : nat -> val_UniformSpace) (ul : val_UniformSpace),
             is_lim_seq un ul ->
             (forall n, p_def_lambda_prop_modal_free_one_fixed i G (un n)) ->
             p_def_lambda_prop_modal_free_one G ul).
  { intros.
    apply p_lambda_prop_modal_free_one_seq_fixed in X0 as [[phi i] [Hsubseq Hipp]].
...
(** Case of Theorem 9 where the complexity of G has the shape (0,0,c) *)
...
Proof.
  intros.
  apply lambda_prop_modal_free_inv.
  - apply p_hseq_basic_hseq_basic; apply X.
  - assert (H' := modal_complexity_eval_p_hseq val_l G).
    lia.
  - apply def_lambda_prop_modal_free_one_implies_reg.
    apply def_lambda_prop_modal_free_one_p_implies_reg.
    apply (p_lambda_prop_modal_free_one_lim G val_n); try assumption.
    intros n.
    specialize (X0 n).
    apply def_lambda_prop_modal_free_one_reg_implies_p.
    apply def_lambda_prop_modal_free_reg_implies_one.
    apply lambda_prop_modal_free.
    + apply p_hseq_basic_hseq_basic; apply X.
    + assert (H' := modal_complexity_eval_p_hseq (val_n n) G).
      lia.
    + apply X0.
Qed.

(** Case of Theorem 9 where the complexity of G has the shape (0,b,c) *)
Lemma HMR_no_diamond_lim : forall G (val_n : nat -> val_UniformSpace) (val_l : val_UniformSpace),
    ...
    HMR_T_M (map (eval_p_sequent val_l) G).
Proof.
  intros G; remember (HMR_outer_complexity_p_hseq G).
  revert G Heqp.
  ...
  - apply HMR_atomic_lim with val_n; try assumption.
    apply p_hseq_is_basic_outer_complexity_0_inv.
    rewrite <- Heqn; reflexivity.
  - remember (apply_logical_rule_on_p_hypersequent (p_hseq_put_non_basic_fst G)).
    destruct s.
    + specialize (Hinn (HMR_outer_complexity_p_hseq p)).
      apply p_hseq_put_non_basic_fst_HMR.
      { intros H.
        apply p_hseq_is_basic_outer_complexity_0 in H.
        rewrite <- Heqn in H.
        inversion H. }
      ...
      * apply Hinn with val_n.
        -- rewrite Heqn.
           symmetry in Heqs.
           assert (complexity_p_hseq p <3 complexity_p_hseq G).
           { apply apply_logical_rule_on_p_hypersequent_correct_inl with a; try assumption.
             simpl.
             rewrite <- Heqn; reflexivity. }
           unfold complexity_p_hseq in H.
           inversion H; subst.
           ++ rewrite Hnd in H1; inversion H1.
           ++ rewrite surjective_pairing.
              rewrite (surjective_pairing (HMR_outer_complexity_p_hseq p)).
        ...
        -- intros n.
           ...
           ++ apply p_hseq_put_non_basic_fst_HMR_inv.
              { intros H; apply p_hseq_is_basic_outer_complexity_0 in H.
    ...
    + destruct p as [G1 G2].
      ...
      * apply p_hseq_put_non_basic_fst_well_defined.
        { rewrite <- Heqn.
          simpl; lia. }
        apply p_hseq_well_defined_lim with val_n; try assumption.
      * apply Hinn with (HMR_outer_complexity_p_hseq G1) val_n.
        -- rewrite Heqn.
           symmetry in Heqs.
           assert (complexity_p_hseq G1 <3 complexity_p_hseq G).
           ...
           ++ rewrite Hnd in H1; inversion H1.
           ++ rewrite surjective_pairing.
              rewrite (surjective_pairing (HMR_outer_complexity_p_hseq G1)).
              apply fst_lt2; assumption.
           ++ rewrite surjective_pairing.
              rewrite (surjective_pairing (HMR_outer_complexity_p_hseq G1)).
              rewrite H5.
        ...
        -- intros n.
           ...
           ++ apply p_hseq_put_non_basic_fst_HMR_inv.
              { intros H; apply p_hseq_is_basic_outer_complexity_0 in H.
                rewrite <- Heqn in H; inversion H. }
              apply Hpi_n.
      * apply Hinn with (HMR_outer_complexity_p_hseq G2) val_n.
        -- rewrite Heqn.
           symmetry in Heqs.
           assert (complexity_p_hseq G2 <3 complexity_p_hseq G).
           ...
           ++ rewrite Hnd in H1; inversion H1.
           ++ rewrite surjective_pairing.
              rewrite (surjective_pairing (HMR_outer_complexity_p_hseq G2)).
              apply fst_lt2; assumption.
           ++ rewrite surjective_pairing.
              rewrite (surjective_pairing (HMR_outer_complexity_p_hseq G2)).
              rewrite H5.
...
(** Theorem 9 *)
(** p_lambda_prop_one_lim is the basic case of the proof and HMR_lim is the full Theorem 9 *)
Lemma p_lambda_prop_one_lim : forall G (un : nat -> val_UniformSpace) (ul : val_UniformSpace) (acc : Acc lt_nat4 (complexity_p_hseq G , 0%nat)) i,
    (forall n, p_hseq_well_defined (un n) G) ->
    is_lim_seq un ul ->
    (forall n, p_def_lambda_prop_one_fixed i G (un n)) ->
    p_def_lambda_prop_one G ul
with HMR_lim : forall G (val_n : nat -> val_UniformSpace) (val_l : val_UniformSpace) (acc : Acc lt_nat4 (complexity_p_hseq G , 1%nat)),
    (forall n, p_hseq_well_defined (val_n n) G) ->
    is_lim_seq val_n val_l ->
    (forall n, HMR_T_M (map (eval_p_sequent (val_n n)) G)) ->
    HMR_T_M (map (eval_p_sequent val_l) G).
Proof.
  ...
  destruct acc as [acc].
  remember (modal_complexity_p_hseq G) as nb_diam.
  destruct nb_diam.
  { destruct (p_lambda_prop_modal_free_one_lim G un ul Hlim) as [L [Hlen [[[H' Hborned] Hsum] Hone]]].
    - intros n.
      ...
      + split with L.
        ...
        { assert (H'' :=modal_complexity_eval_p_hseq ul G).
          lia. }
  ...
  - destruct Hcs as [Hsubseq [Hlenul Hlimul]].
    ...
    + rewrite concat_eval_p_sequent.
      ...
      * apply acc.
        ...
        rewrite Nat.max_0_r.
        rewrite <- Heqnb_diam.
        apply modal_complexity_p_seq_concat.
        rewrite Heqnb_diam.
        rewrite modal_complexity_p_hseq_mul_new_var.
        apply Forall_inf_modal_complexity_only_diamond_p_hseq.
        rewrite <- modal_complexity_p_hseq_mul_new_var.
        lia.
      * intros n.
      ...
      * intros n.
        ...
        apply Hstep.
(* HMR_lim *)
  intros G; remember (complexity_p_hseq G).
  ...
  - apply lambda_prop_inv.
    { apply p_hseq_basic_hseq_basic.
      unfold modal_complexity_p_hseq in Heqp.
      inversion Heqp.
      apply p_hseq_is_basic_outer_complexity_0_inv; lia. }
    apply def_lambda_prop_one_implies_reg.
    apply def_lambda_prop_one_p_implies_reg.
    destruct (p_lambda_prop_one_seq_fixed G val_n) as [[phi i] [Hsubseq H]].
    { intros n.
    ...
    + apply is_lim_seq_subseq with val_n; try assumption.
      split with phi; split; auto.
    + intros n; specialize (H n); apply H.
  - remember (apply_logical_rule_on_p_hypersequent (p_hseq_put_non_basic_fst G)).
    destruct s.
    + apply p_hseq_put_non_basic_fst_HMR.
      { intros H.
...
Lemma FreeMRS_archimedean : forall A B,
    (forall n, (INRpos n) *S A <== B) ->
    A <== MRS_zero.
Proof.
  intros A B H.
  apply leq_cong_l with (-S (-S A)); [auto with MGA_solver | ].
  apply leq_cong_r with (-S MRS_zero); [ auto | ].
  ...
  { simpl.
    ...
    auto. }
  eapply hmr_sound.
  apply HMR_archimedean with B.
  intros n.
  apply hmrr_can_elim.
  eapply HMR_le_frag; [ | apply hmr_complete].
  - repeat split; auto.
